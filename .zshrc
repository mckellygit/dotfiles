...
# less version 608+ with -F is like smartless ...
...
alias smesg='VLESS_AT_END=1 vless /var/log/syslog'
alias vsmesg='VLESS_AT_END=1 vimless /var/log/syslog'
alias nsmesg='VLESS_AT_END=1 nvimless /var/log/syslog'
...
alias cmakereldbg='CC="ccache gcc" CXX="ccache g++" cmake -DCMAKE_BUILD_TYPE=RelWithDebInfo -DUSE_LIBMEMCACHED=OFF -DUSE_LIBXSLT=ON -DUSE_CPPUNIT=ON  -DUSE_AWS=OFF -DUSE_CASSANDRA=OFF -DUSE_JAVA=ON -DSUPPRESS_JAVAEMBED=OFF -DINCLUDE_JAVAEMBED=ON -DUSE_SHLIBDEPS=OFF -DCMAKE_EXPORT_COMPILE_COMMANDS=1 -DCPACK_CMAKE_GENERATOR=Ninja -DECLWATCH_BUILD_STRATEGY=IF_MISSING ../HPCC-Platform ; pushd ../HPCC-Platform && git clean -f -fd ./esp/src ; rm -rf ./esp/src/node_modules ./esp/src/lib ./esp/src/build ; popd ; ccache -C'
...
alias cmakemysqlplug='CC="ccache gcc" CXX="ccache g++" cmake -DCMAKE_BUILD_TYPE=RelWithDebInfo -DUSE_LIBMEMCACHED=OFF -DUSE_LIBXSLT=ON -DUSE_CPPUNIT=ON -DUSE_AWS=OFF -DUSE_CASSANDRA=OFF -DUSE_SHLIBDEPS=OFF -DUSE_MYSQL=ON -DINCLUDE_PLUGINS=ON -DSUPPRESS_MYSQLEMBED=OFF -DSUPPRESS_REMBED=ON -DSUPPRESS_V8EMBED=ON -DSUPPRESS_MEMCACHED=ON -DSUPPRESS_REDIS=ON -DSUPPRESS_SQS=ON -DUSE_JAVA=ON -DSUPPRESS_JAVAEMBED=OFF -DINCLUDE_JAVAEMBED=ON -DSUPPRESS_SQLITE3EMBED=ON -DSUPPRESS_KAFKA=ON -DSUPPRESS_COUCHBASEEMBED=ON -DSUPPRESS_SPARK=ON -DSUPPRESS_EXAMPLEPLUGIN=ON -DCMAKE_EXPORT_COMPILE_COMMANDS=1 -DCPACK_CMAKE_GENERATOR=Ninja -DECLWATCH_BUILD_STRATEGY=IF_MISSING ../HPCC-Platform ; pushd ../HPCC-Platform && git clean -f -fd ./esp/src ; rm -rf ./esp/src/node_modules ./esp/src/lib ./esp/src/build ; popd ; ccache -C'
alias cmakerdln='cmake -DCMAKE_BUILD_TYPE=RelWithDebInfo -DUSE_LIBMEMCACHED=OFF -DUSE_LIBXSLT=ON -DUSE_CPPUNIT=ON  -DUSE_AWS=OFF -DUSE_CASSANDRA=OFF -DUSE_JAVA=ON -DSUPPRESS_JAVAEMBED=OFF -DINCLUDE_JAVAEMBED=ON -DUSE_SHLIBDEPS=OFF -DCMAKE_EXPORT_COMPILE_COMMANDS=1 -DCPACK_CMAKE_GENERATOR=Ninja -DECLWATCH_BUILD_STRATEGY=IF_MISSING ../LN'
...
export FZF_ALT_C_OPTS="--scheme=path --bind='shift-up:half-page-up' --bind='shift-down:half-page-down' --bind='alt-K:half-page-up,alt-J:half-page-down' --bind='ctrl-alt-k:half-page-up,ctrl-alt-j:half-page-down' --bind='ctrl-alt-o:half-page-down' --bind='pgup:page-up' --bind='pgdn:page-down'"
...
export FZF_DEFAULT_OPTS='--height 40% --ansi --preview "$BATNAME --style=numbers --color=always --line-range :250 {}" --bind="alt-q:cancel" --bind="ctrl-alt-p:toggle-preview" --bind="alt-g:preview-top,alt-G:preview-bottom" --bind="ctrl-f:half-page-down" --bind="ctrl-b:half-page-up" --bind="ctrl-k:up,ctrl-j:down" --bind="ctrl-d:backward-delete-char" --bind="shift-up:preview-half-page-up" --bind="shift-down:preview-half-page-down" --bind "home:pos(-1),end:pos(0)" --bind="alt-up:offset-up+up,alt-down:offset-down+down" --bind="alt-k:offset-up+up,alt-j:offset-down+down" --bind="alt-K:preview-half-page-up,alt-J:preview-half-page-down" --bind="ctrl-alt-k:preview-half-page-up,ctrl-alt-j:preview-half-page-down" --bind="ctrl-alt-o:preview-half-page-down" --bind="pgup:preview-page-up" --bind="pgdn:preview-page-down" --bind="alt-d:kill-word,alt-u:unix-line-discard,shift-delete:kill-line,ctrl-delete:clear-query" --bind="alt-b:page-up" --bind="alt-f:page-down" --preview-window=right:hidden --color fg:242,bg:236,hl:65,fg+:15,bg+:239,hl+:108 --color info:108,prompt:109,spinner:108,pointer:168,marker:168'
...
fzf-history-widget() {
  ...
  selected="$(fc -l 1 | awk '{ cmd=$0; sub(/^[ \t]*[0-9]+\**[ \t]+/, "", cmd); if (!seen[cmd]++) print $0 }' |
    FZF_DEFAULT_OPTS="--height ${FZF_TMUX_HEIGHT:-40%} $FZF_DEFAULT_OPTS -n2..,.. --scheme=history --bind=ctrl-r:abort,ctrl-z:ignore $FZF_CTRL_R_OPTS --query=${(qqq)LBUFFER} +m" $(_fzfcmd))"
  local ret=$?
  if [ -n "$selected" ]; then
...
my-fzf-history-widget() {
  ...
  setopt localoptions noglobsubst noposixbuiltins pipefail no_aliases 2> /dev/null
  selected="$(fc -l 1 | awk '{ cmd=$0; sub(/^[ \t]*[0-9]+\**[ \t]+/, "", cmd); if (!seen[cmd]++) print $0 }' |
    FZF_DEFAULT_OPTS="--height ${FZF_TMUX_HEIGHT:-40%} $FZF_DEFAULT_OPTS -n2..,.. --scheme=history --bind=ctrl-r:abort,ctrl-z:ignore $FZF_CTRL_R_OPTS --query=${(qqq)LBUFFER} +m" $(my-fzfcmd))"
  local ret=$?
  if [ -n "$selected" ]; then
    num=$(awk '{print $1}' <<< "$selected")
    if [[ "$num" =~ ^[1-9][0-9]*$ ]]; then
...
alias manls="man -k . | fzf --tac --bind=\"ctrl-f:half-page-down\" --bind=\"ctrl-b:half-page-up\" --bind=\"ctrl-k:up,ctrl-j:down\" --bind=\"ctrl-d:backward-delete-char\" --bind \"home:pos(-1),end:pos(0)\" --bind=\"alt-up:offset-up+up,alt-down:offset-down+down\" --bind=\"alt-k:offset-up+up,alt-j:offset-down+down\" --bind=\"ctrl-alt-k:half-page-up,ctrl-alt-j:half-page-down\" --bind=\"ctrl-alt-o:half-page-down\" --bind=\"pgup:page-up\" --bind=\"pgdn:page-down\" --bind=\"alt-d:kill-word,alt-u:unix-line-discard\" --bind=\"alt-b:page-up\" --bind=\"alt-f:page-down\" --prompt='Man> ' | awk '{print \$1}' | xargs -r man -P 'sh -c \"col -bx | $BATNAME -l man -pp | less\"'"
...
bindkey "\e\x0c" noop
# NOTE: M-C-l in some terminals could be to clear screen AND scrollback buffer
